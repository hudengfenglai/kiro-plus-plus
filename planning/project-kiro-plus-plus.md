...
- Added a global historical-bundle banner at the top of the workbench so users can immediately see when they are reading an old support snapshot instead of the current live environment, with direct actions to return, open the recorded request file, or copy a shareable snapshot.
- Made the top-bar primary action, Hero setup actions, and launch entry historical-bundle aware, so clicking them from an old support snapshot now first returns to the latest live bundle before running real proxy / setup / launch actions.
- Locked the right-rail Playground into a clear historical-reference mode when browsing an old support bundle, so users now see an explicit “resume live playground” path instead of accidentally treating archived context as a real-time validation surface.
- Added the same historical-source treatment to the Diagnostics rail, so the whole right column now consistently signals when its summaries and detail links come from an archived support bundle rather than the current live environment.
- Extended historical-mode signaling into the center workbench tabs as well, so logs, output, and diagnostics now explain how their content relates to the selected support snapshot instead of silently mixing live-session data with archived context.
- Added a local Markdown export for the current workbench state, so operators can save the exact support/share view they are looking at into `exports/` and attach it directly to GitHub or LinuxDO without manual copy-paste loss.
- Persisted the latest workbench Markdown snapshot into desktop settings and surfaced a direct reopen action in the Diagnostics area, so the new export path remains visible and reusable across refreshes or app restarts.
- Extended workbench Markdown exports into a small local history with reopen / remove / clear actions, so these share artifacts are now managed more like support bundles instead of acting as a single transient file pointer.
- Added missing-file detection for persisted workbench snapshots, so the Diagnostics area now marks stale snapshot records when the Markdown file was manually deleted on disk and gives users a clear cleanup path instead of a mysterious open-path failure.
- Added the same missing-file detection to persisted support bundles, so stale diagnostics bundle history now surfaces as “文件缺失” in the workbench instead of pretending the bundle is still fully usable after someone deletes files on disk.
- Added one-click cleanup for missing support-bundle and workbench-snapshot records, so users can prune only the stale history entries without wiping healthy diagnostics history alongside them.
- Enriched missing support-bundle diagnostics with the exact missing file keys (for example `summaryPath` or `zipPath`), so stale bundle cards now explain what is broken instead of only saying the bundle is unavailable.
- Split support-bundle availability from zip availability, so deleting only the `.zip` file no longer makes the whole bundle look broken; the workbench now correctly treats the summary/snapshot bundle as usable while separately flagging that the shareable zip needs re-export.
